...
// Version 1.1 has (OTC Ingredients) and (Precursors) -[INGREDIENTS_OF]-> (Operation:Mix)
...
//
// ------------------------------------------------------------------------------------
// (OTC Ingredient) -[INGREDIENT_OF]-> (Mix)  //  Consider merging INGREDIENT_OF with WENT_TO
// (Ing 1)
LOAD CSV WITH HEADERS FROM ($mix_makepre_link) AS row
WITH row
CALL {
  ...
  MERGE (source)-[r: `INGREDIENT_OF`]->(target)
}; //IN TRANSACTIONS OF 10000 ROWS;  Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
// (Ing 2)
LOAD CSV WITH HEADERS FROM ($mix_makepre_link) AS row
WITH row
...
//
// (Precursor) -[INGREDIENT_OF]-> (Operation:Mix:MakeSample)
// (Ing 1)
LOAD CSV WITH HEADERS FROM ($mix_makesamp_link) AS row
WITH row
CALL {
