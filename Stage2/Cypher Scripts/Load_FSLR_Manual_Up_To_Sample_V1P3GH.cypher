...
// Version 1.3
    //Added timeline relationships between operations on a shared object node.
    //Added Status as property to Sample node.
    //Added Target Molarity for Ing1 and Ing2 in Precursor Node.
...
CALL {
  ...
  SET n.`Ing 1 Description` = row.`Ing 1 Description`
  SET n.`Ing 1 ID` = row.`Ing 1 ID`
  SET n.`Ing 1 Target Molarity` = row.`Ing 1 Target Molarity`
  SET n.`Ing 2 Description` = row.`Ing 2 Description`
  SET n.`Ing 2 ID` = row.`Ing 2 ID`
  SET n.`Ing 2 Target Molarity` = row.`Ing 2 Target Molarity`
}; //IN TRANSACTIONS OF 10000 ROWS;  Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
//
// HEAT OPERATION
// --------------
...
//
// Operation Timelines.  For each individual object node not 'ITC Ingredient', create -[THEN]-> relationship between all operations in chronological order.
