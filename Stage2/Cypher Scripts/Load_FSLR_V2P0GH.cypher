...
// Should one day separate the code into several files and call from within the code (if that's possible)
// Meant to be run as an explicit transaction (until I can figure out how to run an implicit one)
// Partially generated by Neo4j Workshop.  Adapted to run in Neo4j Browser.  Tested on a localhost server.  Successfully run to AuraDB.
// Version 2.0
    //Attempted Simplification of Excel Sheets by adding an OTC Ingredient Sheet and removing redundant information of mixing operations.
    //Had to recode to match new changes.
...
// Copy Paste VIEWABLE link
:param users_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_UsersF.csv";
:param OTC_ingredients_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_OTC_Ingredients.csv";
:param mix_makepre_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_op_MixMakePre.csv";
:param heat_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_op_Heat.csv"; 
:param rest_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_op_Rest.csv";
:param mix_makesamp_link => "https://raw.githubusercontent.com/lteresah/AMLS-GraphDatabase/main/Stage2/CSVs_Version2/Neo4j_op_MixMakeSamp.csv";
:param idsToSkip: [];
...
//
// Users
CREATE CONSTRAINT `kerberos User_uniq` IF NOT EXISTS
FOR (n: `User`)
...
// CALL was included in generated code but its usage in LOADCSV is not mentioned in documentation.
// It is possible CALL is just useful for implementing more complicated subqueries on each row.
LOAD CSV WITH HEADERS FROM ($users_link) AS row
WITH row
WHERE NOT row.`kerberos` IN $idsToSkip AND NOT row.`kerberos` IS NULL
CALL {
  WITH row
  MERGE (n: `User` { `kerberos`: row.`kerberos` })
...
// ----------------
LOAD CSV WITH HEADERS FROM ($OTC_ingredients_link) AS row
WITH row
WHERE NOT row.`OTC ID` IN $idsToSkip AND NOT row.`OTC ID` IS NULL
CALL {
  WITH row
  MERGE (n:`OTC Ingredient` { `OTC ID`: row.`OTC ID` })
  SET n.`OTC ID` = row.`OTC ID`
  SET n.`Description` = row.`OTC Description`
  SET n.`Formula` = row.`OTC Formula`
  SET n.`State` = row.`OTC State`
  SET n.`Purity` = toFloat(trim(row.`OTC Purity`))
  SET n.`Molar Mass (mg)` = toInteger(trim(row.`OTC Molar Mass (mg)`))
}; // IN TRANSACTIONS OF 10000 ROWS; Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
//
...
CALL {
  ...
  SET n.`Heat Object Description` = row.`Heat Object Description`
  SET n.`Dissolved` = row.`Heat Dissolve`
  SET n.`Temp (C)` = toFloat(trim(row.`Heat Temp (C)`))
  SET n.`Time (min)` = toFloat(trim(row.`Heat Time (min)`))
  SET n.`Executor` = row.`Executor`
  SET n.`Notes` = row.`Notes`
}; // IN TRANSACTIONS OF 10000 ROWS; Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
//
...
CALL {
  ...
  SET n.`Time (min)` = toFloat(trim(row.`Rest Time (min)`))
  SET n.`Executor` = row.`Executor`
  SET n.`Notes` = row.`Notes`
}; // IN TRANSACTIONS OF 10000 ROWS; Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
//
// OPERATION:MIX:MAKESAMPLE
// --------------
...
//
// ------------------------------------------------------------------------------------
// (OTC Ingredient) -[WENT_TO]-> (Operation:Mix:MakePrecursor)  //  Consider merging WENT_TO with WENT_TO
// (Ing 1)
LOAD CSV WITH HEADERS FROM ($mix_makepre_link) AS row
WITH row
CALL {
  WITH row
  MATCH (source: `OTC Ingredient` { `OTC ID`: row.`Ing 1 ID` })
...
CALL {
  WITH row
  MATCH (source: `OTC Ingredient` { `OTC ID`: row.`Ing 2 ID` })
  MATCH (target: Operation:Mix:MakePrecursor { `Mix Operation ID`: row.`Mix Operation ID` })
  MERGE (source)-[r: `WENT_TO`]->(target)
}; //IN TRANSACTIONS OF 10000 ROWS;  Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
// --------------------
// (OTC Ingredient) -[WENT_TO]-> (Operation:Mix:MakeSample)
// (Ing 1)
LOAD CSV WITH HEADERS FROM ($mix_makesamp_link) AS row
WITH row
CALL {
...
// ------------------
// (Precursor) -[WENT_TO]-> (Operation:Mix:MakePrecursor)
// (Ing 1)
LOAD CSV WITH HEADERS FROM ($mix_makepre_link) AS row
WITH row
CALL {
  WITH row
  MATCH (source: `Precursor` { `Precursor_ID`: row.`Ing 1 ID` })
  MATCH (target: Operation:Mix:MakePrecursor { `Mix Operation ID`: row.`Mix Operation ID` })
  MERGE (source)-[r: `WENT_TO`]->(target)
};
// (Ing 2)
...
CALL {
  WITH row
  MATCH (source: `Precursor` { `Precursor_ID`: row.`Ing 2 ID` })
  MATCH (target: Operation:Mix:MakePrecursor { `Mix Operation ID`: row.`Mix Operation ID` })
  MERGE (source)-[r: `WENT_TO`]->(target)
};
// -------------------
...
//
// ----------------------------------------------------------------------------
// (User) -[EXECUTED]-> (Operation:Mix:MakePrecursor)
LOAD CSV WITH HEADERS FROM ($mix_makepre_link) AS row
WITH row
CALL {
  WITH row
  MATCH (source: `User` { `kerberos`: row.`Executor` })
  MATCH (target: Operation:Mix:MakePrecursor { `Mix Operation ID`: row.`Mix Operation ID` })
  MERGE (source)-[r: `EXECUTED`]->(target)
}; //IN TRANSACTIONS OF 10000 ROWS;   Does not work in external transaction.  Semicolon Required if removing IN TRANSACTIONS.
//
// (User) -[EXECUTED]-> (Operation:Mix:MakeSample)
LOAD CSV WITH HEADERS FROM ($mix_makesamp_link) AS row
WITH row
CALL {
  WITH row
...
// -----------------------------------------------------------------------------------
// Operation Timelines.  For each individual object node not 'OTC Ingredient', create -[THEN]-> relationship between all operations in chronological order.
// This one's a little different.
// Find all nodes that are not `OTC Ingredient` and do not contain Operation as a label
MATCH (n) WHERE NOT n:Operation AND NOT n:`OTC Ingredient`
// Within the subgroup, find ones that went_to an operation
MATCH (n)-[r:WENT_TO]->(op:Operation)
// Order by chronological order of operations.  ORDER BY requires WITH (or other statement) to proceed
WITH n,op ORDER BY op.`Timestamp`
// Collect all the operations belonging to each node
WITH n, COLLECT(op) AS Operations
// Confusign nested for-loop to add relationships
FOREACH (i IN RANGE(0,size(Operations)-2)|
    FOREACH (before in [Operations[i]]|
        FOREACH(after in [Operations[i+1]]|
