...
I had to manually specify the operation type when creating the Python lists from the Operation pandas DataFrames. This was necessary because the operation type was not explicitly provided in a column, and I wanted to avoid using regex to extract it. Since we plan to transition away from using CSVs, this might become irrelevant. However, the same consideration should apply to lists automatically generated by automated equipment.
